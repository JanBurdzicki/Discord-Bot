...
class Poll(Base):
    ...
    id = Column(Integer, primary_key=True, autoincrement=True)
    poll_id = Column(String, unique=True, nullable=False)  # Unique public ID
    question = Column(String, nullable=False)
    options = Column(String, nullable=False)  # Comma-separated options
    creator_id = Column(BigInteger, nullable=False)  # Discord user ID
    ...
    is_advanced = Column(Boolean, default=False)  # For advanced polls
    external_id = Column(String, nullable=True)  # For advanced polls (e.g., StrawPoll ID)
...
class Vote(Base):
    """Model for storing poll votes."""
    __tablename__ = 'votes'

    id = Column(Integer, primary_key=True, autoincrement=True)
    poll_id = Column(String, ForeignKey('polls.poll_id', ondelete='CASCADE'), nullable=False)
    user_id = Column(BigInteger, nullable=False)  # Discord user ID
    option_index = Column(Integer, nullable=False)  # 0-based index of the chosen option
